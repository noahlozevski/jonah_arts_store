<template lang='pug'>
    .containerfancy(@mouseover="selected()" @mouseleave="unSelected()" :style="{ cursor: selectedCursor }")
        //- template(v-if="!hovered")
        transition(name='fade')
            img.unhovered(:src="mouseOff" v-if="!hovered" :style="imgStyle")
        transition(name='fade')
            img.hovered(:src="mouseOn" v-if="hovered" :style="imgStyle")
</template>
...
export default {
    data () {
        return {
            hovered: false,
            hConstrain: { height: `100%`, width: `auto`, display: `block` },
            vConstrain: { height: `auto`, width: `100%`, display: `block` },
        }
    },
    props: {
    ...
    computed: {
        ...
        imgStyle () {
            if (this.v) return this.vConstrain;
            else return this.hConstrain
        }
    }
